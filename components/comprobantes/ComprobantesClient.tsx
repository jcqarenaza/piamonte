...
export default function ComprobantesClient({ userId, rol = 'ventas' }: { userId:string; rol?:string }) {
  ...
  async function solicitarCAE(c: Comprobante, tipoCbteOverride?: number, cbteAsoc?: {tipo:number; ptoVta:number; nro:number}): Promise<boolean> {
    ...
    try {
      ...
      // Red bidireccional — la LETRA del comprobante manda:
      // Clase A (FA=1, ND=2, NC=3, FCE=201/202/203): receptor RI por definición.
      // Clase B: jamás receptor RI.
      const CLASE_A = [1, 2, 3, 201, 202, 203]
      if (CLASE_A.includes(tipoCbte)) condIvaReceptor = 1
  ...
  async function save(){
    ...
    // Contador atómico — evita race condition en numeración
    const { data: nextNumData } = await supabase.rpc('siguiente_numero_comprobante', { p_tipo: tipoDoc(), p_categoria: 'factura' })
    const nextNum = nextNumData as number
  ...
  async function generarPDF(c:Comprobante): Promise<Blob> {
    ...
    // ─── QR FISCAL ARCA — misma URL, generación, POSICIÓN y TAMAÑO que el formato ARCA
    // (route generar-pdf-arca: 80x80pt en x:20 y:55 → 28,2mm en x:7,1 y:249,4 de un A4) ───
    ...
    if(c.pagos?.length && !esCuentaCorriente){
      ...
      doc.text('Forma de pago:', xL, pagoY)
      ...
      const pagoLines = doc.splitTextToSize(pagoTexto, rw - 32 - (xL - pad))
      doc.text(pagoLines.slice(0,2), xL + 28, pagoY)
      doc.setFontSize(8)
    }
    ...
    if (c.observaciones) {
      ...
      doc.text('Obs.:', xL, totY + 24)
      doc.setFont('helvetica','normal')
      const obsLines = doc.splitTextToSize(c.observaciones, rw - 16 - (xL - pad))
      doc.text(obsLines.slice(0, 3), xL + 12, totY + 24)
      doc.setFontSize(8)
    }
    ...
    if (!c.es_negro && ['A','B','C'].includes(c.tipo) && c.cae_emitido) {
      doc.setDrawColor(210,220,215); doc.setLineWidth(0.3)
      rRect(xL, caeY, W - pad - xL, 10, 2, 'S')
      doc.setFont('helvetica','bold'); doc.setFontSize(8); doc.setTextColor(30,30,30)
      doc.text(`CAE: ${c.cae_emitido}`, xL+4, caeY+6.5)
  ...
  async function generarPDFArca(c:Comprobante): Promise<Blob> {
    ...
    // Llamar a la API route Node.js que usa pdf-lib
    const res = await fetch('/api/generar-pdf-arca', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ comprobante: c, cuitAseg, dirAseg, razonSocial }),
    })
    if (!res.ok) throw new Error('Error generando PDF: ' + await res.text())
